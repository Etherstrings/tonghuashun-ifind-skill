...
2. `quote-realtime`、`quote-history`、`market-snapshot`、`fundamental-basic` 以及涨停数据查询是常见问题的稳定路由。
...
6. 对涨停数据查询，如果 iFinD 不可用，会自动回退到东方财富公开涨停池接口。
7. `fundamental-basic` 目前不做公开源兜底；如果 iFinD 不可用，就明确告诉用户当前 skill 未覆盖公开源基本面能力。
...
- 个股最新价、个股近一段时间走势、大盘快照、基础财务指标、涨停数据查询，优先用 `smart-query`。
...
- 涨停数据查询会先走 iFinD；如果 iFinD 查询失败，再自动尝试东方财富公开涨停池。
