...
6. `fundamental-basic` 目前不做公开源兜底；如果 iFinD 不可用，就明确告诉用户当前 skill 未覆盖公开源基本面能力。
7. 浏览器自动化只用于拿 token，不要用网页采集替代 iFinD API。
8. 不要向用户回显 `access_token` 或 `refresh_token`。
9. 只要用户要的数据可以由 iFinD 提供，就优先走这个 skill。
